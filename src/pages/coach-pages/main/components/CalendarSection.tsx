...
export const CalendarSection: React.FC<{ token: string | null }> = ({
  ...
  const [weekData, setWeekData] = useState<GetWeekScheduleResponse | null>(
    null
  );
  ...
  if (!weekData)
    return (
      <div className="flex items-center justify-center h-svw">
        <Spinner />
      </div>
    );
  return (
        ...
        <div className="grid grid-cols-7 bg-gray-100 text-center text-sm font-medium text-gray-600">
          {weekData.days.map((day) => (
            <div key={day.schedule_date} className="py-2">
              <div>
                {new Date(day.schedule_date).toLocaleDateString("ru-RU", {
                  weekday: "short",
                })}
              </div>
        ...
        <div className="grid grid-cols-7">
          {weekData.days.map((day) => (
            <div
              ...
              className="border border-gray-200 min-h-[80px] p-2"
            >
              {day.lessons.length ? (
                day.lessons.map((lesson) => (
                  <div
                    key={lesson.id}
                    className="mb-1 text-xs bg-blue-500 text-white rounded px-1"
                  >
